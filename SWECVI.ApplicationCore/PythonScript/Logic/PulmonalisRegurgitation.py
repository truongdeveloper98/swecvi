...
def PulmonalisRegurgitation(Parameters, References):

    dCodeResult = DCodeResult()
    dCodeResult.Result = False
    dCodeResult.PAR = Parameters
    dCodeResult.CallLevel = 220
    ...
    for it in Parameters:
          if(it['Name'] == ParameterNames.PRERO):
             ERO = it['Value']
          elif(it['Name'] == ParameterNames.PRRV):
             RV = it['Value']
    
    for it in References:
          if(it['ParameterNameLogic'] == ParameterNames.PRERO):
             EROReference = it
          elif(it['ParameterNameLogic'] == ParameterNames.PRRV):
             RVReference = it
    ...
    if(EROReference['MildlyAbnormalRangeLower'] == None or
       ...
       EROReference['ModeratelyAbnormalRangeLower'] == None):
             dCodeResult.DCode = GetValueByPRRV(RV, RVReference)
             return dCodeResult
    else:
...
def GetValueByPRRV(RV, RVReference):
